const EmployeesSection = {
  ...
  async render(container, { user }) {
    ...
    container.innerHTML = `
      <div class="toolbar">
        ...
        <div class="gap-8">
          <span class="segmented" role="group" aria-label="Tampilan direktori">
            <button class="btn btn-outline btn-sm on" id="view-grid" aria-pressed="true">Grid</button>
            <button class="btn btn-outline btn-sm" id="view-list" aria-pressed="false">List</button>
          </span>
          ${canEdit ? `<button class="btn btn-outline btn-sm" id="import-emp">Import Excel</button>` : ''}
          ${canEdit ? `<button class="btn btn-primary btn-sm" id="add-emp">Tambah Karyawan</button>` : ''}
        </div>
      </div>
    ...
    const setView = (view) => {
      ...
      document.getElementById('view-grid').classList.toggle('on', grid);
      document.getElementById('view-list').classList.toggle('on', !grid);
      document.getElementById('view-grid').setAttribute('aria-pressed', grid);
      document.getElementById('view-list').setAttribute('aria-pressed', !grid);
      this.load();
    };
  ...
  async load() {
    ...
    if (res.data.length === 0) {
      body.innerHTML = `<div class="empty-state"><div class="empty-title">Tidak ada karyawan yang cocok.</div></div>`;
    ...
    return `<div class="grid grid-4">${rows.map((e) => `
      <div class="card" style="cursor:pointer;text-align:center;" data-open-profile="${e.id}">
        <div class="emp-photo" style="width:56px;height:56px;margin:0 auto 10px;font-size:16px;">
          ${e.photo_url && safeUrl(e.photo_url) ? `<img src="${escapeHtml(safeUrl(e.photo_url))}" />` : initials(e.full_name)}
        </div>
    ...
    return `<div class="card table-wrap"><table class="data-table">
      ...
      <tbody>${rows.map((e) => `
        <tr style="cursor:pointer" data-open-profile="${e.id}">
          <td><div class="emp-cell"><div class="emp-photo">${e.photo_url && safeUrl(e.photo_url) ? `<img src="${escapeHtml(safeUrl(e.photo_url))}" />` : initials(e.full_name)}</div>
    ...
    openDrawer({
      ...
      bodyHtml: `
        <div style="text-align:center;margin-bottom:16px;">
          <div class="emp-photo" style="width:72px;height:72px;margin:0 auto 10px;font-size:20px;">${emp.photo_url && safeUrl(emp.photo_url) ? `<img src="${escapeHtml(safeUrl(emp.photo_url))}" />` : initials(emp.full_name)}</div>
      ...
      onMount: () => {
            ...
            ${canEdit ? `
              ...
              <div class="gap-8 mt-16">
                ...
                ${account
                  ...
                  : `<button class="btn btn-primary btn-sm" id="create-account-emp">Buat Akun Login</button>`}
            ...
            document.getElementById('reset-pw-emp')?.addEventListener('click', async () => {
              ...
              try {
                ...
                openModal({
                  ...
                  footHtml: `<button class="btn btn-primary" data-close-modal>Selesai</button>`,
                });
        ...
        const renderDocs = () => {
          body.innerHTML = `
            ${data.documents.length ? data.documents.map((d) => { const href = safeUrl(d.file_url); return href ? `<div class="flex-between" style="padding:6px 0;"><a href="${escapeHtml(href)}" target="_blank" rel="noopener">${escapeHtml(d.file_name)}</a><span class="muted small">${escapeHtml(d.type)}</span></div>` : ''; }).join('') : emptyLine('Belum ada dokumen')}
        ...
        const renderHistory = async () => {
          ...
          const typeLabel = { onboarding: 'Onboarding', promotion: 'Promosi', transfer: 'Mutasi', offboarding: 'Offboarding', contract_change: 'Perubahan Kontrak', status_change: 'Perubahan Status' };
          let rows;
          try { rows = await api(`/employees/${emp.id}/history`); } catch (e) { body.innerHTML = escapeHtml(e.message); return; }
          body.innerHTML = `
            ${canEdit ? `<button class="btn btn-primary btn-sm mt-16" id="add-history" style="margin-bottom:12px;">Catat Promosi/Mutasi/Offboarding</button>` : ''}
  ...
  openForm(existing = null) {
    ...
    openModal({
      ...
      `,
      footHtml: `<button class="btn btn-outline" data-close-modal>Batal</button><button class="btn btn-primary" id="save-emp">Simpan</button>`,
      onMount: () => {
  ...
  openHistoryForm(emp, onSaved) {
    ...
    openModal({
      ...
      `,
      footHtml: `<button class="btn btn-outline" data-close-modal>Batal</button><button class="btn btn-primary" id="hist-save">Simpan</button>`,
      onMount: () => {
  ...
  openImport() {
    openModal({
      ...
      `,
      footHtml: `<button class="btn btn-outline" data-close-modal>Batal</button><button class="btn btn-primary hidden" id="import-confirm">Import Sekarang</button>`,
      onMount: () => {
            ...
            box.innerHTML = parsedRows.length
              ? `<p class="small mt-16"><strong>${parsedRows.length}</strong> baris siap diimport.</p>
                 <div class="table-wrap" style="max-height:200px;overflow-y:auto;"><table class="data-table">
                   ...
                   <tbody>${parsedRows.slice(0, 20).map((r) => `<tr><td>${escapeHtml(r.employeeCode)}</td><td>${escapeHtml(r.fullName)}</td><td>${r.departmentId ? 'Cocok' : '<span class=\"muted\">tidak cocok</span>'}</td><td>${r.positionId ? 'Cocok' : '<span class=\"muted\">tidak cocok</span>'}</td></tr>`).join('')}</tbody>
                 </table></div>`
              : `<p class="small mt-16" style="color:var(--danger)">Tidak ada baris valid (Employee Code dan Full Name wajib ada).</p>`;
  ...
  openCreateAccountForm(emp, onCreated) {
    ...
    openModal({
      ...
      `,
      footHtml: `<button class="btn btn-outline" data-close-modal>Batal</button><button class="btn btn-primary" id="acc-save">Buat Akun</button>`,
      onMount: () => {
        document.getElementById('acc-save').addEventListener('click', async () => {
          ...
          try {
            ...
            openModal({
              ...
              bodyHtml: `<p class="small muted">Sampaikan kredensial ini ke karyawan secara aman. Password ini hanya ditampilkan satu kali.</p>
                <div class="field"><label>Email</label><input value="${escapeHtml(res.email)}" disabled /></div>
                <div class="field"><label>Password Sementara</label><input value="${escapeHtml(res.temporaryPassword)}" disabled style="font-family:ui-monospace,Menlo,Consolas,monospace;" /></div>
